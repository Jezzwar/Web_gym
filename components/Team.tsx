...
export default function Team() {
  ...
  // Reset progress when trainer changes
  useEffect(() => {
    tickRef.current = 0
    setProgress(0)
    setIsEnded(false)
    ...
    <section className={styles.section} id="community">
      ...
      <div className={styles.inner}>
        <div className={styles.header}>
          <p className="section-label">The Professionals</p>
        ...
        <div className={styles.avatarRow}>
          {trainers.map((t, i) => (
            <button key={t.id} className={styles.avatarBtn} onClick={() => openStory(i)}>
              ...
              <span className={styles.avatarId}>{t.id}</span>
              <span className={styles.avatarName}>{t.name}</span>
              <span className={styles.avatarRole}>{t.role}</span>
            </button>
          ))}
        </div>
      </div>
            ...
            >
              ...
              <div className={styles.storyHeader}>
                ...
                <div className={styles.storyActions}>
                  <button className={styles.actionBtn} onClick={handleControl} aria-label="pause/play">
                    {isPaused
                      ? isEnded ? <RotateCcw size={16} /> : <Play size={16} />
                      : <Pause size={16} />}
                  </button>
                  <button className={styles.actionBtn} onClick={close} aria-label="close">
                    <X size={16} />
                  </button>
                </div>
              </div>
